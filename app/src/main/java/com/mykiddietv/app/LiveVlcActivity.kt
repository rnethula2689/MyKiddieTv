...
class LiveVlcActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        b.pipBtn.visibility = if (!isArchive && !kidMode && !tv) View.VISIBLE else View.GONE
        b.recBtn.visibility = if (!kidMode) View.VISIBLE else View.GONE
     ...
     *  dead, so rebuild the player from scratch and re-resolve a fresh link. */
    ...
    private fun toggleRecord() {
        ...
        if (!isRecording) {
            ...
            try { p.record(null) } catch (_: Exception) {}
        }
    }
    ...
    private fun showMenu() {
        ...
        val items = if (kidMode)
            ...
            arrayOf("🔄   Retry stream", "⚠   Report not working", SleepTimer.menuLabel(), "🎚   Playback settings", "⚙   Settings", "📥   App updates", "ℹ️   About", "✖   Exit")
    ...
    /** Movie/episode-in-VLC menu (parent side) — full parity with the ExoPlayer VOD menu (plus a
     *  Speed item, since VLC has no on-screen speed button) so switching engines loses no option. */
        ...
        val dlg = AlertDialog.Builder(this)
            .setItems(items) { _, which ->
                ...
                when {
                    ...
                    action.contains("Audio boost") -> cycleVlcAudioBoost()
                    action.contains("speed") -> showVodSpeedDialog()
                    action.contains("Subtitles") -> searchSubtitles()
                    action.contains("Report") -> reportVod()
                    action.contains("Playback settings") -> PlaybackSettings.show(this)
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        if (event.action == KeyEvent.ACTION_DOWN) {
            ...
            if (isArchive) {
                ...
                if (barShown && event.keyCode in OPTION_NAV_KEYS) scheduleHide()
                when (event.keyCode) {
                    KeyEvent.KEYCODE_MENU -> { showMenu(); return true }
