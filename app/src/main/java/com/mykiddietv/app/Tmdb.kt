...
/**
 * Minimal TMDb client: movie/series metadata, trailers and cast. Key comes from BuildConfig.TMDB_KEY
 * (injected from the CI secret); when blank, callers fall back gracefully.
 */
object Tmdb {
    ...
    /** One search call → poster + rating + overview for a movie. */
    ...
    /** Returns a YouTube video id for the best trailer, or null. */
    fun trailerYoutubeId(apiKey: String, title: String, year: String): String? {
        if (apiKey.isBlank() || title.isBlank()) return null
        return try {
            val movieId = searchId(apiKey, "movie", title, year)
                ?: (if (year.isNotBlank()) searchId(apiKey, "movie", title, "") else null) ?: return null
    ...
    private fun bestTrailerKey(apiKey: String, movieId: Int): String? {
        ...
        for (i in 0 until vids.length()) {
            ...
            var score = when (v.optString("type")) { "Trailer" -> 4; "Teaser" -> 2; else -> 0 }
