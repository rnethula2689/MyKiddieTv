...
import com.mykiddietv.app.databinding.ActivityMovieDetailBinding
 ...
 * All actions live here; replaces the old popup. Adapts portrait/landscape automatically.
 */
class MovieDetailActivity : AppCompatActivity() {
    ...
    private fun selectSeason(s: Portal.Season) {
        ...
        io.execute {
            val eps = Portal.seriesEpisodes(vodId, s.id).reversed()
            runOnUiThread {
    ...
    /** The kid app has no per-episode portal stills (Portal.Episode carries no screenshot), so every
     *  episode card shows the same series poster. Swap in the real per-episode stills from TMDb — the
     *  same source Strimix shows. */
