...
object Updater {
    private const val REPO = "rnethula2689/MyKiddieTv"
    ...
    fun latest(): Pair<Int, String>? {
        return try {
            val req = Request.Builder().url(versionUrl).header("User-Agent", "MyKiddieTv").build()
    ...
    fun downloadApk(dest: File): Boolean {
        return try {
            val req = Request.Builder().url(apkUrl).header("User-Agent", "MyKiddieTv").build()
            client.newCall(req).execute().use { r ->
