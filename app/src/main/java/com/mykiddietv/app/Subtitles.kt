...
object Subtitles {
    private const val APP_UA = "MyKiddieTv v1.0"
    ...
    /** Plain subtitle search by title in [lang]. [year] (e.g. "2026") narrows results to that release
     *  so same-title films of other years are excluded — OpenSubtitles matches by title alone otherwise.
     *  Blank year = title-only. */
    fun search(query: String, year: String = "", lang: Lang = LANGS[0]): List<Sub> =
        if (apiKey.isNotBlank()) searchApi(query, year, lang.api2) else searchLegacy(query, year, lang.legacy3)
    ...
    private fun searchApi(query: String, year: String, lang2: String = "en"): List<Sub> {
        val q = URLEncoder.encode(query, "UTF-8")
        val yp = if (year.isNotBlank()) "&year=" + URLEncoder.encode(year.trim(), "UTF-8") else ""
        return apiFetch("https://api.opensubtitles.com/api/v1/subtitles?languages=$lang2&order_by=download_count&query=$q$yp")
    }
    ...
    private fun searchLegacy(query: String, year: String, lang3: String = "eng"): List<Sub> {
        val out = ArrayList<Sub>()
        try {
