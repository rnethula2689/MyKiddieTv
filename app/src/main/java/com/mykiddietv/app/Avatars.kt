...
/**
 * Kid profile avatars. A profile's `avatar` string is one of:
 *   ""              -> default (coloured circle + name initial)
 *   "emoji:🦁"      -> a built-in fun icon
 *   "file:/path"    -> a photo the parent took or picked (stored square ~256px)
 */
object Avatars {
    /** Built-in kid-friendly picks: animals, then toys / fun stuff. */
    val EMOJI = listOf(
        "🦁","🐶","🐱","🦊","🐻","🐼","🐨","🐯","🦄","🐸","🐵","🐰",
        "🐧","🐢","🦋","🐙","🐳","🦖","🦕","🐝","🐞","🦉","🐬","🐠",
        "🚗","🚂","✈️","🚀","🚁","⚽","🏀","🎈","🧸","🎮","🎨","🎸",
    ...
    fun circularBitmap(path: String, sizePx: Int): Bitmap? = try {
        ...
        // Subsample large photos to avoid OOM.
