...
/**
 * Content-customization profiles (Prime/YouTube style) for the PARENT side. Each profile chooses
 * which Live TV categories (genres) and which Movie/VOD categories are visible, so e.g. a "Telugu"
 * profile only sees Telugu live + movie folders. Device-local; the active one is remembered.
 * (Distinct from [Profiles], which is the kid-mode whitelist/passcode store.)
 */
